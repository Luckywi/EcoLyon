...
// MARK: - BancsMapView Style Apple Maps iOS 26

struct BancsMapView: View {
    @Environment(\.colorScheme) var colorScheme
    @StateObject private var bancService = MapDataService(configuration: BancsConfiguration())
    ...
    @State private var selectedBanc: BancLocation?
    ...
    // Couleur thème bancs
    private let themeColor = Color(red: 0.6, green: 0.45, blue: 0.35)
    ...
    private var showLoadingOverlay: Bool {
        (!hasLoadedOnce && bancService.items.isEmpty) || hasLoadingError
    }

    private var hasLoadingError: Bool {
        bancService.errorMessage != nil && bancService.items.isEmpty
    }
    ...
    private var nearbyBancs: [BancLocation] {
        guard let refLoc = currentFocusLocation else { return bancService.items }

        return bancService.items
            .map { banc in
    ...
    private var topThreeBancs: [BancLocation] {
        return Array(nearbyBancs.prefix(5))
    }
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ...
                MapHeaderIsland(
                    ...
                    description: "Cette carte affiche jusqu'à 50 bancs publics dans un rayon de 800 m autour de votre position ou de votre recherche.",
        ...
        .onChange(of: locationService.isLocationReady) { _, isReady in
            if isReady, let location = locationService.userLocation, !isSearchMode {
                ...
                Task {
                    await bancService.loadAroundLocation(location)
                    await weatherService.fetchWeather(for: location)
                    hasLoadedOnce = true
                }
            }
        }
        .onChange(of: bancService.items.count) { _, newCount in
            if newCount > 0 {
                hasLoadedOnce = true
            }
        }
        ...
        .sheet(item: $selectedBanc) { banc in
            BancDetailSheet(banc: banc, userLocation: currentFocusLocation)
        ...
        VStack(spacing: 0) {
            ...
            HStack(spacing: 12) {
                HStack(spacing: 8) {
                    ...
                    TextField("Bancs", text: $searchText)
                        .font(.system(size: 16))
                    ...
                    VStack(spacing: 0) {
                        if !searchText.isEmpty && searchedLocation != nil {
                            ...
                            if !topThreeBancs.isEmpty {
                                VStack(spacing: 0) {
                                    ForEach(Array(topThreeBancs.prefix(3).enumerated()), id: \.element.id) { _, banc in
                                        Button(action: {
                                            selectBancFromList(banc)
                                        }) {
                                            bancRowContent(banc: banc)
                                        }
                                        .buttonStyle(.plain)
                                    }
                                }
                            } else {
                                MapEmptyStateView(imageName: "bench", message: "Aucun banc à proximité")
                            }
                            ...
                            // Section raccourcis
                            MapShortcutsSection(currentDestination: .bancs)
                        }
                    }
                    .padding(.bottom, 20)
                }
    ...
    @ViewBuilder
    private func bancRowContent(banc: BancLocation) -> some View {
        HStack(spacing: 14) {
            Image("Banc")
                .resizable()
                .aspectRatio(contentMode: .fit)
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                if let refLoc = currentFocusLocation {
                    Text("\(formatDistance(from: refLoc, to: banc.coordinate)) · \(banc.address)")
                        .font(.system(size: 14))
                        .foregroundColor(.secondary)
                        .lineLimit(1)
                }
    ...
    private var modernMap: some View {
        Map(position: $cameraPosition, interactionModes: [.pan, .zoom, .rotate, .pitch]) {
            ...
            ForEach(nearbyBancs) { banc in
                Annotation("", coordinate: banc.coordinate) {
                    ModernMarkerView(imageName: "Banc", size: 32) {
                        selectBancFromMap(banc)
                    }
                }
            }
    ...
    private func selectBancFromMap(_ banc: BancLocation) {
        UIImpactFeedbackGenerator(style: .light).impactOccurred()
        withAnimation(IslandState.animation) {
            isSheetExpanded = false
            isSearchFocused = false
    ...
    private func selectBancFromList(_ banc: BancLocation) {
        UIImpactFeedbackGenerator(style: .light).impactOccurred()
        openDirections(to: banc)
    }

    private func openDirections(to banc: BancLocation) {
        openDirectionsTo(coordinate: banc.coordinate, name: "Banc")
    }

    private func setupInitialLocation() {
        if let userLocation = locationService.userLocation {
            Task {
                await bancService.loadAroundLocation(userLocation)
                hasLoadedOnce = true
            }
        } else {
    ...
    private func handleSuggestionTap(_ suggestion: AddressSuggestion) {
        ...
        Task {
            if let coordinate = await GeocodingService.shared.getCoordinate(for: suggestion) {
                ...
                await bancService.loadAroundLocation(coordinate)
...
// MARK: - Banc Detail Sheet

struct BancDetailSheet: View {
    ...
    private var distanceText: String {
        ...
        return formatDistance(from: userLoc, to: banc.coordinate)
    }
